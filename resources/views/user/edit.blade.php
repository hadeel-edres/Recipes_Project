    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex m-2 p-2">
                <a href="{{ route('user.show', ['user_id' => $userrecipe->user_id, 'id' => $userrecipe->id]) }}" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Zurück</a>
            </div>
                ...
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('user.update',['user_id' => $user->id, 'userrecipe' => $userrecipe->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="text" id="name" name="name" value="{{ old('name', $userrecipe->name) }}" class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 @error('name') border-red-400 @enderror" />
                            </div>
                            @error('name')
                        ...
                        <div class="sm:col-span-6 pt-5">
                            ...
                            <div class="mt-1">
                                <textarea id="body" rows="3" name="description" class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('description') border-red-400 @enderror">{{ $userrecipe->description }}</textarea>
                            </div>
                            @error('description')
                            <div class="text-sm text-red-400">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mt-6 p-4">
                            <button type="submit" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Aktualisieren</button>
                        </div>
                    </form>
                </div>
            </div>
